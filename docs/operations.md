...
6. 运行桌面安装程序，首次打开直接填写部署者提供的服务地址，例如 `https://infohub.example.com`；可先测试连接，再点击“保存并连接”。之后用主密码登录。地址会保存到 `%APPDATA%\local.zhistack.infohub\config.json`，下次启动自动读取；设置页可查看完整路径。

配置文件中的 `serverUrl` 为完整 HTTP/HTTPS API 地址，保存时自动补齐末尾 `/api`。首次安装没有默认服务器；升级自 0.2.0 时，旧缓存仅预填地址，需要保存一次。配置文件不包含密码或会话令牌。连接失败时可在登录页打开“服务连接设置”修改；文件无法读取或内容损坏时，也会重新显示填写页面。写入失败保留原配置和当前输入。
