...
## 2026-09-11 产品完善与 0.2.0

完整发布门禁 20 步通过，包含 104 项单元/组件检查及 91 组真实 PostgreSQL 集成。新增版本与回收站、草稿、完整备份/恢复/密码轮换、项目与搜索整理、后台收录/订阅、API 环境/历史/取消、引导和使用反馈。维护 CLI 已通过真实服务备份及干净实例恢复；旧库测试重建第一版结构后执行全部迁移。详见 [本版说明与源码摘要](release-0.2.0.md) 和 [逐项完成情况](product-completion.md)。

本版未完成真实 GUI、Docker、跨机器安装、云主机/R2 和真实参与者试用。浏览器工具连接报 `unsupported Codex auth method: apikey`，本机无 Docker Engine；其他待验项目需对应环境。以下 2026-09-10 及更早记录是历史证据，不作为本版 GUI 验收依据。

Windows release 服务端、桌面 EXE 及 NSIS 构建通过，桌面资源版本为 0.2.0。额外使用发布包服务端执行 8 组生命周期集成通过；包内前端内容、维护 CLI 的备份与空实例恢复通过。ZIP 内 23 个文件（含清单）数量正确，22 个载荷大小与 SHA-256 全部匹配。产物为 `release/infohub-0.2.0-windows-x64.zip`，12,397,566 字节，SHA-256 为 `50537699ada5f238071a0a3a4481154dfedfc2bf06c1cb06883df21d15a36892`；另附 `.sha256`。原始产物报告为 `.local/verification/artifacts.json`。
