...
import type { Endpoint, Item, ItemData, ProbeResult } from '../types';
import { sizeLabel } from '../types';
import { api, copyText, downloadBlob, errorMessage, exportText, saveItem } from '../lib/api';
  ...
  async function send() {
    ...
    try {
      ...
      const result = await api<ProbeResult>('/probe', {
        method: 'POST',
        body: JSON.stringify(buildRequest(data, ep)),
      });
      if (selectionVersion.current === selection) setResponse(result);
    } catch (e) {
      if (selectionVersion.current === selection) setError(errorMessage(e));
    } finally {
      if (sendToken.current === token) setSending(false);
    }
  }
  async function copy() {
    try {
              ...
              {ep && (
                <section className="request-editor">
                  ...
                  <div className="tabs parameter-tabs">
                    <button
                      className={epTab === 'headers' ? 'active' : ''}
                      onClick={() => setEpTab('headers')}
                    >
