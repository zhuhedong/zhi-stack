...
当前版本为 **0.2.0**。八类产品问题的实现及验收见 [产品完善清单](docs/product-completion.md)和[本版说明](docs/release-0.2.0.md)。完整能力见 [功能清单](docs/feature-list.md)，安装、备份、升级与回退见 [运维手册](docs/operations.md)。
...
脚本创建随机命名的独立测试数据库、隔离文件目录及本地 HTTP 测试站点，覆盖认证、分页搜索、并发冲突、密文、锁定权限、附件边界、离线图片、失败清理和重启持久化。完整发布检查还执行生命周期、项目整理、API 历史、后台任务、订阅、反馈与维护 CLI 的集成流程。结束后只删除脚本自己创建的数据，不修改业务库。组件回归验证草稿保护、异步保存、接口切换和取消等交互；当前数量及环境限制以 [本版记录](docs/release-0.2.0.md) 为准。
